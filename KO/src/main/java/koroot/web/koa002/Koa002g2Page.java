...
public class Koa002g2Page extends PageBaceClass {
	...
	public Class<?> doFinishUpdate() {
		...
		//ステータス
		ety_kogu.setPaymentFlag(sutatus);
		
		//支払日
		ety_kogu.setPaymentDate(CommonUtil.getNowUpdDate());
		//支払社員ID
		ety_kogu.setPaymentEmpid(this.sessionDto.EmployeeID);
		//更新日
	...
	public Class<?> initialize() {
		...
		try{
			...
			//申請日
			sinseidate = ety_kogu.getKyokaKeiriDate();
			//勘定科目
			kanjo = ety_kan.getName();
			//許可/不許可
			sutatus = ety_kogu.getPaymentFlag();
			//金額
